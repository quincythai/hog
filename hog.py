...
def roll_dice(num_rolls, dice=six_sided):
    ...
    total = 0
    pig_out = False # rolling 1 will return 1.
    while num_rolls >= 1:
        roll = dice()
        if roll == 1:
            pig_out = True
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # free bacon: if player chooses 0 dice, they score the largest
    # digit of the opponents score + 1.
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    # Swine swap: if at the end of a turn, one player's score is double the
    # other player's, then swap scores.
    ...
    while score < 100 and opponent_score < 100:
        if who == 0: # my turn
            selected_dice = select_dice(score, opponent_score)
            num_dice_to_roll = strategy0(score, opponent_score)
            score += take_turn(num_dice_to_roll, opponent_score, selected_dice)
        elif who == 1: # opponent's turn
            selected_dice = select_dice(opponent_score, score)
            num_dice_to_roll = strategy1(opponent_score, score)
            opponent_score += take_turn(num_dice_to_roll, score, selected_dice)
        ...
        if score == 2 * opponent_score or opponent_score == 2 * score: 
            score, opponent_score = swap(score, opponent_score)
...
def make_averaged(fn, num_samples=1000):
    ...
    # *args - can take any # of arguments
    ...
    make_test_dice simulates a dice with the sequence (3, 1, 5, 6), and returns the function to dice
    dice() -> 3, dice() -> 1, dice() -> 5, dice() -> 6

    make_averaged(dice, 1000) returns  function that executes dice() 1000 times and returns result
    
    make_averaged(roll_dice, 1000)(2, dice)
    will roll_dice 1000 times with arguments 2 and the dice we set with specific sequence
    Result is the average of the scores of rolling 2 dice with (3, 1, 5, 6)
...
def bacon_strategy(score, opponent_score):
    ...
    # Free bacon: if user rolls 0 dice, they will get largest digit of
    # opponent's score + 1.

    free_bacon_score = max(opponent_score // 10, opponent_score % 10) + 1
    
    # if doing Free bacon would give at least BACON_MARGIN points, then roll 0.
    if free_bacon_score >= BACON_MARGIN:
        return 0
    # otherwise rolling 0 is detrimental
    # (< BACON_MARGIN), just roll BASELINE_NUM_ROLLS
    else:
...
def swap_strategy(score, opponent_score):
    ...
    # Swine swap: if after a roll, one player's score is double the other player's, swap scores.
    
    # Free bacon: if user rolls 0 dice, they get the largest digit
    # of opponents score + 1 added to their score.
    free_bacon_score = max(opponent_score // 10, opponent_score % 10) + 1

    # opponent's score is obviously larger, so roll 0.
    if score + free_bacon_score == opponent_score // 2:
        return 0
    # opponent's score is smaller, we do not want to roll 0 (roll baseline)
    elif score + free_bacon_score == opponent_score * 2:
        return BASELINE_NUM_ROLLS
    # No swine swap, check if rolling 0 is >= BACON_MARGIN?
    elif free_bacon_score >= BACON_MARGIN:
        return 0
    # No other beneficial option, just roll BASELINE_NUM_ROLLS
    else:
        return BASELINE_NUM_ROLLS  # otherwise, roll baseline
    
def final_strategy(score, opponent_score):

    # Score gotten from Free_bacon.
    ...
    # if we DON't swap scores.
    if score + bacon_score != opponent_score * 2:
        if winning:
            if ((score + opponent_score + bacon_score) % 7 == 0):
                rolls = 0
            elif bacon_score > BACON_MARGIN:
                rolls = 0
            elif score - opponent_score >= 40:
                rolls = 0
            elif score >= 85:
                rolls = 0
        else: # score + bacon_score == opponent_score * 2
            if 2 * (score + bacon_score) == opponent_score:
                rolls = 0
            elif bacon_score > BACON_MARGIN:
                rolls = 0
            elif (score + opponent_score + bacon_score) % 7 == 0:
                rolls = 0
